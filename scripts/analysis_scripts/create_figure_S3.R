...
props <- props %>%
  mutate(logit_props_spontan = car::logit(props_spotan)) 

prop_spontan_lm <- lmer(
  logit_props_spontan ~ treatment + season + (1|site_code), 
  data = props
)
...
spontan_emm_trt <- emmeans(
  prop_spontan_lm, 
  "treatment", 
  lmer.df = "satterthwaite"
)

pairs_spontan_trt <- as.data.frame(pairs(spontan_emm_trt))

# obtain p-values for comparison between
# undisturbed and tilled
pairs_til_res <- pairs_spontan_trt %>%
...
pairs_mow_res <- pairs_spontan_trt %>%
...
pairs_mow_til <- pairs_spontan_trt %>%
  filter(contrast == "TIL - MOW") %>%
  pull(p.value) %>%
  signif(digits = 2)

# visualize the data ----
(prop_spontan_plot <- props_data_viz %>%
   ggplot(aes(x = treatment, y = props_spotan, fill = season)) +
   geom_boxplot() + 
   ylim(0, 3) + 
   ...
   labs(
     x = "Management Regime", 
     y = "Proportion of Spontaneous Species"
   ) + 
...
ggsave(
  filename = here("output", "results", "figure-S3.svg"), 
  device = "svg", 
  units = "in",
  height = 5, 
  width = 7
